  ...
  <div class="view dashboard">
    <div class="view-header">
      <div>
        ...
        <p class="view-sub">{{ local('Канал, статистика и управление роликами в одном месте', 'Channel, stats and video manager in one place') }}</p>
      </div>
      <div class="header-actions">
        <button class="btn-secondary" @click="refreshAll" :disabled="yt.isLoading">
      ...
      <div class="toolbar card">
        <div class="search-box">
          ...
          <div class="search-row">
            <input v-model="query" :placeholder="t('videos.searchPlaceholder')" @keyup.enter="reloadVideos" />
            <button class="btn-secondary" @click="reloadVideos" :disabled="yt.isLoading">
              {{ t('common.search') }}
            </button>
        ...
        <div class="amount-box">
          ...
          <select v-model="maxResults" @change="reloadVideos">
        ...
        <div class="details-panel card">
          ...
          <template v-else>
            ...
            <div class="detail-thumb-wrap">
              ...
              <button class="btn-secondary detail-thumb-btn" @click="triggerThumbnailPick" :disabled="thumbnailSaving">
                <span v-if="thumbnailSaving" class="spin">↻</span>
                <span v-else>▣</span>
                {{ local('Сменить обложку', 'Change thumbnail') }}
              </button>
              <input ref="thumbInput" type="file" accept="image/*" style="display:none" @change="onThumbnailPick" />
            </div>
            ...
            <div v-if="thumbnailMessage" class="success-msg">{{ thumbnailMessage }}</div>
            <div v-if="error" class="error-msg">{{ error }}</div>

            <button class="btn-primary save-btn" @click="saveVideo" :disabled="yt.isSaving">
...
async function selectVideo(id) {
  ...
  try {
    ...
    router.replace({ path: '/dashboard', query: { id } })
  } catch (e) {
    error.value = e.message
  }
}
...
function triggerThumbnailPick() {
  thumbInput.value?.click()
}

async function onThumbnailPick(e) {
  ...
  try {
    await yt.uploadThumbnail(selected.value.id, file)
    await yt.fetchVideos({ maxResults: maxResults.value, query: query.value }).catch(() => {})
    thumbnailMessage.value = local('Обложка обновлена', 'Thumbnail updated')
  } catch (err) {
...
.stats-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 12px; flex-shrink: 0; }
.stat-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 16px; }
.stat-label { color: var(--text2); font-size: 11px; font-family: var(--font-mono); text-transform: uppercase; letter-spacing: 0.08em; }
...
.detail-thumb-wrap { position: relative; margin-bottom: 12px; }
.detail-thumb { width: 100%; aspect-ratio: 16 / 9; object-fit: cover; border-radius: var(--radius); background: var(--surface2); display: block; }
.detail-thumb-btn { position: absolute; right: 10px; bottom: 10px; background: rgba(22, 22, 42, 0.92); border: 1px solid var(--border2); }
...
.success-msg, .error-msg { font-size: 12px; font-family: var(--font-mono); margin-top: 12px; line-height: 1.5; }
...
@media (max-width: 1180px) { .view { overflow-y: auto; } .stats-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } .content-grid { grid-template-columns: 1fr; } .details-panel { min-height: 620px; } .toolbar { grid-template-columns: 1fr; } }
@media (max-width: 760px) { .stats-grid { grid-template-columns: 1fr; } .video-card { grid-template-columns: 1fr; } .thumb-wrap { width: 100%; height: auto; aspect-ratio: 16 / 9; } .video-side { align-items: flex-start; } .metrics-grid, .form-grid { grid-template-columns: 1fr; } .search-row { flex-direction: column; } .search-row input { min-width: 0; } }
</style>
