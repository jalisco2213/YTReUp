...
async function loadMore() {
  ...
  try {
    await yt.fetchVideos({
      ...
      query: query.value,
      pageToken: yt.videosNextPageToken,
      append: true,
    });
  } catch (e) {
    error.value = e.message;
...
async function onThumbnailPick(e) {
  ...
  try {
    ...
    await yt
      .fetchVideos({ maxResults: maxResults.value, query: query.value })
      .catch(() => {});
...
function privacyClass(s) {
  return `badge badge-${
    s === "public" ? "done" : s === "private" ? "pending" : "downloading"
  }`;
}
...
<template>
  <div class="view dashboard">
    <div class="view-header">
      <div>
        ...
        <p class="view-sub">
          {{
            local(
              "Канал, статистика и управление роликами в одном месте",
              "Channel, stats and video manager in one place"
            )
          }}
        </p>
      </div>
    ...
    <div v-if="!yt.isAuthenticated" class="auth-prompt card">
      ...
      <router-link to="/settings" class="btn-primary link-btn"
        >{{ t("dashboard.openSettings") }} →</router-link
      >
    </div>
    ...
    <template v-else>
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-value accent-purple">
            {{ formatNum(yt.channelStats?.videoCount) }}
          </div>
      ...
      <div class="toolbar card">
        <div class="search-box">
          ...
          <div class="search-row">
            ...
            <button v-if="query" class="btn-ghost" @click="clearSearch">
              {{ t("common.clear") }}
            </button>
          </div>
        </div>
        ...
        <div class="filter-box">
          ...
          <div class="segmented">
            ...
            <button
              :class="{ active: typeFilter === 'shorts' }"
              @click="typeFilter = 'shorts'"
            >
              {{ t("common.shorts") }}
            </button>
          </div>
            ...
            >
              ...
              <div class="video-main">
                ...
                <div class="video-meta">
                  ...
                  <span class="dot">·</span>
                  <span class="mono"
                    >{{ formatNum(video.statistics?.viewCount) }}
                    {{ t("common.views") }}</span
                  >
                  <span class="dot">·</span>
                  <span class="mono"
                    >{{ formatNum(video.statistics?.likeCount) }}
                    {{ t("common.likes") }}</span
                  >
                </div>
              </div>
              ...
              <div class="video-side">
                ...
                <span :class="privacyClass(video.status?.privacyStatus)">{{
                  privacyLabel(video.status?.privacyStatus)
                }}</span>
              </div>
            </div>
        ...
        <div class="details-panel card">
          ...
          <template v-else>
            <div class="details-head">
              ...
              <span :class="privacyClass(selected.status?.privacyStatus)">{{
                privacyLabel(selected.status?.privacyStatus)
              }}</span>
            </div>
            ...
            <div class="detail-thumb-wrap">
              ...
              <button
                class="btn-secondary detail-thumb-btn"
            ...
            <div class="quick-actions">
              <button class="btn-secondary" @click="openOnYoutube(selected.id)">
                {{ t("videos.openOnYouTube") }} →
              </button>
              <button class="btn-secondary" @click="copyLink(selected.id)">
                {{ copied ? t("common.copied") : t("videos.copyLink") }}
              </button>
            </div>
            ...
            <div class="toggles">
              <div class="toggle-row">
                <div>
                  ...
                  <div class="toggle-desc">
                    {{ edit.madeForKids ? t("common.yes") : t("common.no") }}
                  </div>
                </div>
                <button
                  class="toggle"
                  :class="{ on: edit.madeForKids }"
                  @click="edit.madeForKids = !edit.madeForKids"
                >
                  <span />
                </button>
              </div>
              ...
              <div class="toggle-row">
                <div>
                  ...
                  <div class="toggle-desc">
                    {{ edit.embeddable ? t("common.yes") : t("common.no") }}
                  </div>
                </div>
                <button
                  class="toggle"
                  :class="{ on: edit.embeddable }"
                  @click="edit.embeddable = !edit.embeddable"
                >
                  <span />
                </button>
              </div>
              ...
              <div class="toggle-row">
                <div>
                  ...
                  <div class="toggle-desc">
                    {{ edit.ratingsVisible ? t("common.yes") : t("common.no") }}
                  </div>
                </div>
                <button
                  class="toggle"
                  :class="{ on: edit.ratingsVisible }"
                  @click="edit.ratingsVisible = !edit.ratingsVisible"
                >
                  <span />
                </button>
              </div>
