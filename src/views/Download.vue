    ...
    <div class="content">
      ...
      <transition name="slide-up">
        <div v-if="loading" class="card loader-card">
          ...
          <div class="loader-info">
            <div class="loader-title">{{ local('Проверяем ссылку и получаем данные видео', 'Checking link and fetching video info') }}</div>
            <div class="loader-text">
              {{ local('Прошло', 'Elapsed') }}: <span class="mono">{{ formatTime(fetchElapsedSeconds) }}</span>
              <span class="loader-separator">·</span>
              {{ local('примерно осталось', 'approx. left') }}: <span class="mono">{{ formatTime(fetchRemainingSeconds) }}</span>
            </div>
      ...
      <div class="card queue-card">
        <div class="queue-head">
          ...
          <div class="queue-actions">
            <button class="btn-secondary" @click="processAll" :disabled="processing || downloading || !processableCount">
              <span v-if="processing" class="spin">↻</span>
              <span v-else>▶</span>
              {{ t('queue.processAll') }}
            </button>

            <button class="btn-ghost" @click="queue.clearDone" :disabled="!queue.items.length">
              {{ t('queue.clearDone') }}
            </button>
          </div>
        </div>
            ...
            <div class="item-info">
              ...
              <div class="item-progress">
                ...
                <div class="progress-meta">
                  <span class="mono">{{ item.progress || 0 }}%</span>
                  <span class="mono">{{ item.size || '—' }}</span>
                  <span class="mono">{{ item.speed || '—' }}</span>
                  <span class="mono">{{ local('осталось', 'left') }}: {{ queueEta(item) }}</span>
                </div>
              </div>
              ...
              <div class="item-actions">
                <button v-if="canStartItem(item)" class="btn-secondary" @click="downloadQueueItem(item)" :disabled="processing || downloading || processingItemId === item.id">
                  <span v-if="processingItemId === item.id" class="spin">↻</span>
                  <span v-else>↓</span>
                  {{ t('download.download') }}
                </button>

                <button v-if="item.filePath" class="btn-success" @click="goUpload(item.filePath, item.videoInfo?.title)">
                  {{ t('nav.upload') }}
                </button>

                <button v-if="item.filePath" class="btn-ghost" @click="openFolder(item.filePath)">
        ...
        <div v-if="videoInfo" class="card video-preview">
          ...
          <div class="preview-right">
            ...
            <div class="preview-desc">
              {{ videoInfo.description?.slice(0, 220) }}{{ videoInfo.description?.length > 220 ? '…' : '' }}
            </div>
            ...
            <div class="preview-actions">
              <button class="btn-primary" @click="startDownload" :disabled="downloading || processing">
                <span v-if="downloading" class="spin">↻</span>
                <span v-else>↓</span>
                {{ downloading ? t('download.downloading') : t('download.download') }}
              </button>

              <button class="btn-secondary" @click="addToQueue" :disabled="downloading || processing">
                <span>⊞</span>
      ...
      <transition name="slide-up">
        <div v-if="downloading" class="card progress-card">
          ...
          <div class="progress-bar">
            <div class="progress-fill" :style="{ width: `${safePercent(progress.percent)}%` }" />
          </div>

          <div class="download-loader">
            <div class="loader-icon small">
              <span></span>
            </div>

            <div>
              <div class="loader-title">{{ local('Видео скачивается', 'Video is downloading') }}</div>
              <div class="loader-text">
                {{ local('Прошло', 'Elapsed') }}: <span class="mono">{{ formatTime(downloadElapsedSeconds) }}</span>
                <span class="loader-separator">·</span>
                {{ local('осталось', 'left') }}: <span class="mono">{{ downloadEta }}</span>
              </div>
            </div>
          </div>
      ...
      <transition name="slide-up">
        <div v-if="downloadedFile" class="card done-card">
          ...
          <div class="done-actions">
            <button class="btn-success" @click="goUpload(downloadedFile, videoInfo?.title)">
...
const processableCount = computed(() => queue.items.filter(item => ['pending', 'error'].includes(item.status)).length)
  ...
  removeProgress = window.electron?.onProgress(data => {
    ...
    if (activeQueueItemId.value) {
      ...
      queue.updateItem(activeQueueItemId.value, {
        ...
        etaSeconds: estimateRemainingSeconds(normalized.percent, item?.startedAt || Date.now())
      })
...
async function startDownload() {
  ...
  try {
    ...
    progress.value = { percent: 100, size: '', speed: '' }
  } catch (e) {
